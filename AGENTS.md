...
construction. Replay chain verified green across projects; the `rules` project joined after the corpus's last capture, so the whole five-project chain was re-recorded in one pass (r38 lesson: the DRAFT screen turns composer text into a first MESSAGE, not a command — establish a session before exercising command surfaces; and RR2=137: a chatty live loop that records mid-test can be OOM-killed — keep e2e prompts tool-free in WORDS the model obeys). 231 unit/integration.

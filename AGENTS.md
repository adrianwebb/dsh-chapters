...
`elicitedPlot`). e2e is now EIGHT specs in FOUR projects (suite @ 0.75 of the pinned 32K/15K
...
including ANY commit touching an INJECTED file (AGENTS.md/README/skill catalogs): their bytes
ride inside recorded conversations and the tape refuses replay against changed text, by
design; `rm -rf var/model-tape` for a clean corpus). The normalization pipeline is FINAL —
escape/whitespace equivalence classes + volatile rules + tool-result content matched out
(harness-generated bytes carry no model signal); any rule change invalidates all tapes by
construction. Replay chain verified green across ALL FOUR projects (suite/heavy/arrival + `enrich` — the P2 ladder end-to-end replays in 25 s; recording it needed the fork-archive path: a tiny session gives /compact nothing to archive). 215 unit/integration.
...
P3 (rules) remains designed-and-unbuilt.

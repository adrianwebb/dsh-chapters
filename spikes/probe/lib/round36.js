...
export function apply(ctx, config) {
  const run = async () => {
    ...
    for (const q of [
      ...
      'Now src/gitops.ts: which four operations does it wrap, and why does stageAllAndCommit compare workdir files against HEAD instead of using statusMatrix?',
    ...
    const cm = await ctx.commands.execute(child, '/compact', [], new AbortController().signal)
    const cmRes = cm?.result ?? cm
    await new Promise((r) => setTimeout(r, 8000))
    const cMid = await store.get(childId)
    const syncMerged = cMid.chapters.find((c) => ccolls.filter((k) => fullyIn(k, c)).length >= 2)
    record('R3 /compact (ENGINE path) archived a chapter that MERGED both sync collections (live merge #2 through compaction)',
      cmRes?.kind === 'success' && syncMerged !== undefined, {
      result: String(cmRes?.text ?? '').slice(0, 120),
      chapters: cMid.chapters.map((c) => ({ n: c.number, range: [c.startSeq, c.endSeq], title: c.title.slice(0, 34) })),
    ...
    record('R4 final fork: docs archived SEPARATE from the sync chapters (live split)',
      res2?.kind === 'success' && docsHome !== undefined && (syncHome === undefined || docsHome.number !== syncHome.number), {

...
| E2E | The fork BUTTON (client half) | **host half done** (the command-shape path + the `chapters_fork` tool are boot-probed, r24/r26); the client bundle (slots + the client `commands`-service trigger — `connection.rpc.handle` is broken on this host, r26a) is the next work item. The e2e specs land with the button. |

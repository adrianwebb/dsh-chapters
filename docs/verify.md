...
| CACHE | **Cache hit rates rise over time; nothing poisons the prefix silently** | Two nets, both born of the 2026-09-25 machine measurement (a user run showed hit rate FALLING across a session: a compaction span-replacement collapsed cacheRead 29,656→9,613 permanently, and three fan-out children sat pinned at 9,613 while four interleaved sessions thrashed one llama.cpp slot). **Deterministic net** (CI, no GPU): `tape-prefix-stability.test.ts` audits every committed tape — any request that continues a chain must extend its predecessor BYTE-FOR-BYTE; the only tolerated breaks are the two documented replacement points (compaction checkpoint, continuation notice). Message mutation mid-session — the general class that kills prefix caching — fails this ledger with the offending entry. All seven projects pass: the shipped build is byte-stable by evidence, not hope. **Physical net** (opt-in, live): `npm run test:e2e:cache` runs a serial 8-turn session against the real model and asserts the trajectory the design promises — no collapse below 50% of best cacheRead, ≥50% of prompt growth landing cached, final hit rate ≥0.90, no compaction during the probe; every failure prints the full per-turn ledger. It measures PHYSICAL caching, which tapes cannot — and needs a quiet model slot: a pinned-header shape with no compaction means someone else was in the slot (the probe's own first run, taken with a live agent session interleaving on the same llama.cpp slot, is the reference for that signature) |

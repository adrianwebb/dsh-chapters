...
| E2E-TAPE | Acceptance without the GPU | `test:e2e:replay` serves every model turn from `var/model-tape` (proxy smoke: exact + prefix hit, loud 503 miss with journal). Usage fields ride the tape, so the token meter and compaction thresholds fire at identical steps under replay. Tapes are generated artifacts (var/, not committed); regenerate via `test:e2e:record`. The local model remains the distiller and ultimate witness |
| E2E | The fork BUTTON, browser journey | **green (2026-09-19, 7 specs / two projects — suite@0.75-of-32K + arrival@0.9-floor-500 — on a throwaway per-boot home)**: specs CREATE their sessions through the UI ('New session' + one real Local-model turn; turn-complete = the session's OWN `turn/end` in its own log — the global registry count false-passed twice and was retired). Fixture-free by design after the R29-era rows aged out of the 90-session sidebar. The suite also caught the 'Internal Testing Notice' focus trap and the re-link origin bug (see FINDINGS). |
| E2E-EDGE | Oversized turn handled gracefully | **green in-suite (5/5, 16.6m total; 11.4m for this spec) and standalone (run 11: 5 crossings, 27.8m)**: one browser turn reads 700KB range-by-range and crosses the threshold 5×: every mid-turn compaction commits (provider `dsh-chapters`, zero usage), every shadowed seq is chapter-covered, the pre-compaction marker survives in the transcript, the turn completes, and the follow-up turn answers — fork row reappears after settle (observed). The model's plan erosion under heavy trims is the fork's motivation; FINDINGS records it. |
